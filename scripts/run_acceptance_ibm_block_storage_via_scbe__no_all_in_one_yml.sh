...
function basic_tests_on_one_node_sc_pvc_pod_all_in_one()
{
    ...
    echo "NOTE : print the whole files (not all in one file)"
    cat ${ymk_sc_and_pvc_and_pod1}

	echo "####### ---> ${S}. Create SC and PVC first"
    kubectl create -f ${yml_sc_profile}
    kubectl create -f ${yml_pvc}
	echo "## ---> ${S}.1. Verify PVC and PV info status and inpect"
    wait_for_item pvc $PVCName ${PVC_GOOD_STATUS} 10 3
	...
	echo "## ---> ${S}.4 Delete all (POD, SC, PVC, PV and POD)"
    kubectl delete -f ${yml_pod1}
    wait_for_item_to_delete pod $PODName 1120 3
...
function basic_test_POD_with_2_volumes()
{
	...
	echo "####### ---> ${S}. Run SC and PVCs first for the POD"
    kubectl create -f ${yml_sc_profile}
    kubectl create -f ${yml_pvc1}
    kubectl create -f ${yml_pvc2}
	...
	echo "####### ---> ${S}.1.1. create POD (with the 2 PVCs)"
	...
	echo "## ---> ${S}.5 Delete all (POD, 2 PVCs, PV, SC)"
    kubectl delete -f ${yml_pod2}
    wait_for_item_to_delete pod $PODName 1120 3
...
function fstype_basic_check()
{
    ...
    for fstype in ${FS_SUPPORTED}; do
        ...
        yml_pod1=$scripts/../deploy/scbe_volume_with_pod_with_pvc_for_${fstype}.yml
        cp -f ${yml_pod_template} ${yml_pod1}
    ...
    cat ${my_yml}
  	...
  	stepinc
    echo "####### ---> ${S}. Create SC and PVCs first for each fstype"
    for fstype in ${FS_SUPPORTED}; do
        yml_sc_profile=$scripts/../deploy/scbe_volume_storage_class_${profile}_${fstype}.yml
        yml_pvc1=$scripts/../deploy/scbe_volume_pvc_${PVCName}_${fstype}.yml
        kubectl create -f ${yml_sc_profile}
        ...
        echo "## ---> ${S}.1. Verify PVC, PV of $fstype"
    ...
    echo "####### ---> ${S}.1.1 Create PODs in parallel"
    for fstype in ${FS_SUPPORTED}; do
    ...
    echo "####### ---> ${S}.1.2 wait for PODs and check if mounted"
    for fstype in ${FS_SUPPORTED}; do
        echo "## ---> ${S}.1. Verify POD of $fstype"
  	...
  	stepinc
	echo "## ---> ${S}.1.2.1 Delete all pods in parallel"
    for fstype in ${FS_SUPPORTED}; do
        echo "## ---> ${S}.1. Verify PVC, PV and POD of $fstype"
        yml_pod1=$scripts/../deploy/scbe_volume_with_pod_with_pvc_for_${fstype}.yml
        kubectl delete -f ${yml_pod1}
    ...
    echo "####### ---> ${S}.1.2.2 wait for PODs to delete"
    for fstype in ${FS_SUPPORTED}; do
        echo "## ---> ${S}.1. Verify POD of $fstype"
        wait_for_item_to_delete pod ${PODName}-${fstype} 1120 3
    done

	echo "####### ---> ${S}.1.2.3 delete and wait for PVC, SC"
    for fstype in ${FS_SUPPORTED}; do
        yml_sc_profile=$scripts/../deploy/scbe_volume_storage_class_${profile}_${fstype}.yml
        yml_pvc1=$scripts/../deploy/scbe_volume_pvc_${PVCName}_${fstype}.yml
